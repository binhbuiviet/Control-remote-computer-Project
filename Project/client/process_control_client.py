...
def send_kill(client_socket):
    ...
    client_socket.sendall(bytes(str(process_id.get()), "utf8"))
    message = client_socket.recv(BUFSIZ).decode("utf8")
    if "1" in message:
        tk.messagebox.showinfo(message = "Đã đóng process!")
    else:
        tk.messagebox.showerror(message = "Lỗi! Không thể đóng được process.")
    return

def list(client_socket, tab):
    client_socket.sendall(bytes("1", "utf8"))
    list1 = receive(client_socket)
    list1 = pickle.loads(list1)
    list2 = receive(client_socket)
    list2 = pickle.loads(list2) 
    list3 = receive(client_socket)
...
def send_start(client_socket):
    ...
    client_socket.sendall(bytes(str(process_name.get()), "utf8"))
    return
        
def start(root, client_socket):
    process_start = tk.Toplevel(root)
    process_start['bg'] = 'black'
    process_start.geometry("420x50")
    global process_name
    process_name = tk.StringVar(process_start)
    tk.Entry(process_start, textvariable = process_name, width = 38, borderwidth = 5).place(x= 8, y= 20)
    tk.Button(process_start, text = "Start", width = 14, height = 2, fg = 'white', bg = 'dodgerblue', borderwidth=0,
...
def kill(root, client_socket):
    ...
    global process_id
    process_id = tk.StringVar(kill)
    tk.Entry(kill, textvariable = process_id, width = 38, borderwidth = 5).place(x= 8, y= 20)
...
class Process_Control_UI(Frame):
     def __init__(self, parent, client_socket):    
        ...
        parent.geometry("1200x600+200+200")
        ...
        self.tab.heading("Name", text = "Name Process")
        self.tab.heading("ID", text = "ID Process")
        ...
        self.button_list = Button(self, text = 'LIST', width = 20, height = 5, fg = 'white', bg = 'dodgerblue',
            borderwidth=0,
            highlightthickness=0,
            command=lambda: list(client_socket, self.tab),font='Helvetica 12 bold',
            relief="flat"
        )
