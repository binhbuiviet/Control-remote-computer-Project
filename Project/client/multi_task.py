...
class Control(Frame):
    def __init__(self, parent, client_socket, screen_connect, key_connect, mouse_connect):
        ...
        self.info = Button(self, text = 'PRESS ESC TO BACK', width = 20, height = 5,
            borderwidth=0,
            highlightthickness=0,
            fg = 'dodgerblue', bg = '#363636', font='Helvetica 15 bold',
            relief="flat"
            )
        self.info.place(
            x=500,
    ...
    def LiveScreen(self):
        while self.screen_status:
            ...
            while len(image_byte_array) < length_ints:
                buffer = self.screenConnect.recv(BUFSIZ)
                if not buffer:
                    break
